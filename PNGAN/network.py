...
class Res_Generator(nn.Module):
    def __init__(self, ngf, num_resblock):
        ...
        self.conv1 = nn.Sequential(OrderedDict([
            ('pad', nn.ReflectionPad2d(3)),# see in : https://blog.csdn.net/bc521bc/article/details/85134539
            ('conv', nn.Conv2d(6, ngf, kernel_size=7, stride=1, padding=0, bias=True)),# here `out_channel = ngf`
            ('bn', nn.InstanceNorm2d(ngf)),# see in : https://blog.csdn.net/shanglianlm/article/details/85075706
            ('relu', nn.ReLU(inplace=True)),
        ]))
        self.conv2 = nn.Sequential(OrderedDict([
        ...
        for i in range(num_resblock):
            # `setattr(object, name, value)` : set `object`'s `name` and `value`
            setattr(self, 'res'+str(i+1), ResBlock(ngf*4, use_bias=True)) # see in : https://blog.csdn.net/caimouse/article/details/44850539

        self.deconv3 = nn.Sequential(OrderedDict([
            ('deconv', nn.ConvTranspose2d(ngf*4, ngf*2, kernel_size=3, stride=2, padding=1, output_padding=1, bias=True)),# see in : https://blog.csdn.net/zbzcDZF/article/details/87881076
            ('bn', nn.InstanceNorm2d(ngf*2)),
            ('relu', nn.ReLU(True))
        ]))
    ...
    def forward(self, im, pose):
        x = torch.cat((im, pose), dim=1)  # concat image with pose image in channel 
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
...
class ResNet50(nn.Module):
    def __init__(self, num_class=751):
        ...
        net.fc = nn.Sequential()  # remove `fc`
        self.net = net
