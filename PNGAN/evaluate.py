...
def save_images(name, PATH, fake_img):
    ...
    if not os.path.exists(PATH):
        os.mkdir(PATH)
    utils.save_image(samples, '%s/%s.jpg' % (PATH, name), nrow=1, padding=0, normalize=True)
...
if __name__ == '__main__':
    if cfg.TEST_GENERATE == True:
        samples = os.listdir(cfg.TEST.imgs_path)  
        save_path = cfg.TEST.imgs_path.split('/')[-1]  #'bounding_box_train'    
    else:
