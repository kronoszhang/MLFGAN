...
def parse_original_im_name(im_name, parse_type='id'):
  ...
  if parse_type == 'id':
    parsed = -1 if im_name.startswith('-1') else int(im_name[:4]) # id
  else:
    parsed = int(im_name[4]) if im_name.startswith('-1') else int(im_name[6])  # cam
  return parsed
...
def save_images(zip_file, save_dir=None, train_test_split_file=None):
  ...
  im_paths_ = get_im_names(osp.join(raw_dir, 'bounding_box_train'),  # train
  ...
  im_paths_ = get_im_names(osp.join(raw_dir, 'bounding_box_test'), # gallery
                           return_path=True, return_np=False)
  im_paths_.sort()
  ...
  im_paths_ = get_im_names(osp.join(raw_dir, 'query'),  # query
                           return_path=True, return_np=False)
  im_paths_.sort()
  im_paths += list(im_paths_)
  nums.append(len(im_paths_))
  ...
  im_paths_ = get_im_names(osp.join(raw_dir, 'gt_bbox'),  
                           return_path=True, return_np=False)
  im_paths_.sort()
  # Only gather images for those ids and cams used in testing, to form multi-query
...
def transform(zip_file, save_dir=None):
  ...
  # Sort ids, so that id-to-label mapping remains the same when running the code on different machines.
  trainval_ids.sort()
  trainval_ids2labels = dict(zip(trainval_ids, range(len(trainval_ids))))
  partitions = partition_train_val_set(train_test_split['trainval_im_names'], parse_new_im_name, num_val_ids=100)
  train_im_names = partitions['train_im_names']
  train_ids = list(set([parse_new_im_name(n, 'id') for n in partitions['train_im_names']]))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--zip_file', type=str,
                      default='D:\data\dataset\Market_1501\Market-1501-v15.09.15.zip')
