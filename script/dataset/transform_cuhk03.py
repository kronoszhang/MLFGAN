...
def transform(zip_file, train_test_partition_file, save_dir=None):
  ...
  for im_type in ['detected', 'labeled']:
    ...
    train_val_partition = partition_train_val_set(trainval_im_names, parse_im_name, num_val_ids=100)
    train_im_names = train_val_partition['train_im_names']
    train_ids = list(set([parse_im_name(n, 'id') for n in train_val_partition['train_im_names']]))
    # Sort ids, so that id-to-label mapping remains the same when running
    # the code on different machines.
    train_ids.sort()
...
if __name__ == '__main__':
  ...
  parser.add_argument(
    '--zip_file',
    type=str,
    default='D:\data\dataset\CUHK03\cuhk03_release.zip')
  parser.add_argument(
    '--save_dir',
    type=str,
  ...
  parser.add_argument(
    ...
    type=str,
    default='D:\data\dataset\CUHK03\\train_test_split.pkl')  # \ lead error with some reason
