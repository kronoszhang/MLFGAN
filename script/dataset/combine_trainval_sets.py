...
def move_ims(ori_im_paths, new_im_dir, parse_im_name, new_im_name_tmpl, new_start_id):
  ...
  unique_ids.sort()
  id_mapping = dict(zip(unique_ids, range(new_start_id, new_start_id + len(unique_ids))))
...
def combine_trainval_sets(im_dirs, partition_files, save_dir):
  ...
  for im_dir, partition_file in zip(im_dirs, partition_files):
    ...
    im_paths = []
    for n in partitions['trainval_im_names']:
        assert n.__class__.__name__ in ['str','bytes_']
        if n.__class__.__name__ == 'str': # market and duke
            im_paths.append(ospj(im_dir, n))
        # in case of `cuhk03` lead `type(n) = byte` ==> difference between python 2 and 3 
        # `n.decode('utf-8')`==> transform byte to str
        elif n.__class__.__name__ == 'bytes_':  # cuhk03
            im_paths.append(ospj(im_dir, n.decode('utf-8')))
        
    
    im_paths.sort()
    new_im_names_, id_mapping = move_ims(im_paths, new_im_dir, parse_im_name, new_im_name_tmpl, new_start_id)
...
if __name__ == '__main__':
  ...
  cuhk03_im_type = ['detected', 'labeled'][0]  # use detected images
  parser.add_argument(
    '--cuhk03_im_dir',
    type=str,
