...
class Config(object):
  def __init__(self):
    ...
    parser.add_argument('-pmlw', '--pm_loss_weight', type=float, default=1.)# probability mutual loss weight
    parser.add_argument('-gdmlw', '--gdm_loss_weight', type=float, default=1.)# global distance mutual loss weight
    parser.add_argument('-ldmlw', '--ldm_loss_weight', type=float, default=0.)# local distance mutual loss weight
    ...
    # See method `set_devices_for_ml` in `aligned_reid/utils/utils.py` for details.
...
def main():
  ...
  models = [Model(feat_fuse_strategy = cfg.feat_fuse_strategy,
  ...
  def test(train_strategy, norm_base, norm_affine, norm_BaseAffine, feat_fuse_strategy, load_model_weight=False):
    ...
    if cfg.distance_metric != 'None':
      ...
      #metric.fit(global_feats, ids) #################################################################used for other metric
