...
def euclidean_dist(x, y):
  ...
  dist = xx + yy
  dist.addmm_(1, -2, x, y.t())  # compute euclidean distance
...
def hard_example_mining(dist_mat, labels, return_inds=False):
  ...
  if return_inds:
    # shape [N, N]
    ind = (labels.new().resize_as_(labels)  # labels.new()==> define a torch.tensor: tensor([], dtype=torch.int32)
           .copy_(torch.arange(0, N).long())
           .unsqueeze(0).expand(N, N))
    # shape [N, 1]
    p_inds = torch.gather(  # https://blog.csdn.net/lucky_rocks/article/details/79676095
...
def local_loss(
  ...
  if p_inds is None or n_inds is None:  # `p_inds` and `n_inds` are not given by `global distance`, then use `local distance`
                                        # to `hard_example_mining` independently 
  ...
  else: # `p_inds` and `n_inds` are given by `global distance`, means already use `global distance`
        # to `hard_example_mining` , then need not use  `local distance` to `hard_example_mining` again 
