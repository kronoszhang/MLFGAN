...
class TestSet(Dataset):
  ...
  def extract_feat(self, normalize_feat, train_strategy, feat_fuse_strategy, norm_base, norm_affine, norm_BaseAffine):
    ...
    while not done:
      ...
      global_feat, local_feat = self.extract_feat_func(ims_, train_strategy, feat_fuse_strategy, norm_base, norm_affine, norm_BaseAffine)
    ...
    # other fusion strategy can be used in here,like max or avg, weighted diff parts
  ...
  def eval(
    ...
    with measure_time('Extracting feature...'):
      global_feats, local_feats, ids, cams, im_names, marks = \
        self.extract_feat(normalize_feat, train_strategy, feat_fuse_strategy, norm_base, norm_affine, norm_BaseAffine)
    ...
    if distance_metric != 'None':
      global_feats = global_metric.transform(global_feats)  
      # we only use `global_feats` to train `global_metric`, so `global_metric` only can be used to `global_feats`
      #N, H, C = local_feats.shape
      #local_feats = local_metric.transform(local_feats.reshape(N,-1))  # only support 2-dim
      #local_feats = local_feats.reshape(N, H, C)

    with measure_time('Computing global distance...'):
      # query-gallery distance using global distance  # would not use parallel method
      global_q_g_dist = compute_dist(global_feats[q_inds], global_feats[g_inds], type='euclidean')  # `type` choose from `cosine` and `euclidean`
    ...
    if to_re_rank:
      with measure_time('Re-ranking...'):
        # query-query distance using global distance  # would not use parallel method
        global_q_q_dist = compute_dist(global_feats[q_inds], global_feats[q_inds], type='euclidean')

        # gallery-gallery distance using global distance  # would not use parallel method
    ...
    if use_local_distance:
      ...
      local_q_g_dist = low_memory_local_dist( # according to ndim of `local_feats[q_inds], local_feats[g_inds]` 
                                              # to decide whether use would parallel method
                                              # ndim = 2 ==> not use ;  ndim = 3 ==> use
      ...
      if to_re_rank:
        with measure_time('Re-ranking...'):
          ...
          re_r_local_q_g_dist = re_ranking(local_q_g_dist, local_q_q_dist, local_g_g_dist)
  ...
  def demo(
      ...
      to_re_rank=True,
      pool_type='average'):
    """this fun maybe have error!!! this is only for demo.py"""
    with measure_time('Extracting feature...'):
      global_feats, local_feats, ids, cams, im_names, marks = \
        self.extract_feat(normalize_feat, train_strategy, feat_fuse_strategy, norm_base, norm_affine, norm_BaseAffine)
    ...
    with measure_time('Computing global distance...'):
      # query-gallery distance using global distance  # would not use parallel method
      global_q_g_dist = compute_dist(global_feats[q_inds], global_feats[g_inds], type='euclidean')  # `type` choose from `cosine` and `euclidean`
      distmat.append(global_q_g_dist)
    if to_re_rank:
      with measure_time('Re-ranking...'):
        # query-query distance using global distance  # would not use parallel method
        global_q_q_dist = compute_dist(global_feats[q_inds], global_feats[q_inds], type='euclidean')
        ...
        # re-ranked global query-gallery distance
        re_r_global_q_g_dist = re_ranking(global_q_g_dist, global_q_q_dist, global_g_g_dist)
        distmat.append(re_r_global_q_g_dist)
    ...
    if use_local_distance:
      ...
      if to_re_rank:
        with measure_time('Re-ranking...'):
          ...
          local_g_g_dist = low_memory_local_dist(local_feats[g_inds], local_feats[g_inds])
          re_r_local_q_g_dist = re_ranking(local_q_g_dist, local_q_q_dist, local_g_g_dist)
          distmat.append(re_r_local_q_g_dist)
      ...
      #########################
      global_local_q_g_dist = global_q_g_dist + local_q_g_dist
      distmat.append(global_local_q_g_dist)
      if to_re_rank:
        with measure_time('Re-ranking...'):
          global_local_q_q_dist = global_q_q_dist + local_q_q_dist
          global_local_g_g_dist = global_g_g_dist + local_g_g_dist
          re_r_global_local_q_g_dist = re_ranking(global_local_q_g_dist, global_local_q_q_dist, global_local_g_g_dist)
          distmat.append(re_r_global_local_q_g_dist)
    return distmat, im_names[q_inds], im_names[g_inds], im_names[mq_inds], ids[q_inds], ids[g_inds], ids[mq_inds], cams[q_inds], cams[g_inds], cams[mq_inds]
